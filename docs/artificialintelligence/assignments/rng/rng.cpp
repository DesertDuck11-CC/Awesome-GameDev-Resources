...
public:
  ...
  unsigned int lfsr(unsigned int min, unsigned int max) {
    ...
    // The seed is then right-shifted by 1  dropping the least significant bit and then
    // right-shifting all other bits by 1 position. Then the previous bit is left-shifted by 15 bits
